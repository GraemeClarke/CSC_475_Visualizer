...
public class eq_spectrum_avg : MonoBehaviour {
	...
	void Update () {
		...
		float[] avg_spectrum = new float[numberOfObjects];
		int numberOfObjects_countDown = 1024/numberOfObjects;
		...
		if (true) {
			...
			for (int n = 0; n < spectrum.Length; n++) {
				if (numberOfObjects_countDown <= 0) {
					avg_spectrum [j] = temp_avg / numberOfObjects;
					j++;
					temp_avg = 0f;
					numberOfObjects_countDown = 1024/numberOfObjects;
				} else {
					temp_avg += spectrum [n];
					numberOfObjects_countDown--;
				}
			}
		...
		for (int i = 0; i < numberOfObjects; i++) {
			...
			float spectrum_height = avg_spectrum[i] * 400;

			int spectrum_max = 9999;

			if (spectrum_height >= spectrum_max) {
				spectrum_height = spectrum_height* 0.2f;
			}
